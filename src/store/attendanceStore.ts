  ...
  devtools(set => {
    ...
    return {
      ...
      addBatch: rawLogs => {
        ...
        // 3) Flatten + compute status
        const out: AttendanceLog[] = [];
        dayMap.forEach(v => {
          const e = v as AttendanceLog;
          let status: AttendanceLog['status'] = 'Absent';

          // Absent if missing either punch-out
